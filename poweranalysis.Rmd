...
**Questions:**    
- From the available 36 samples (21 for soil 1 and 15 for soil 2), how much power does a suitable test have to determine if there are significant differences between the two soils?  
...
```
Result:  
At a fixed 95% confidence the Kurosol requires more samples as the SE is lower in this soil. So despite that the values in the Chromosol are more variable, less samples are apparently required. Thus one can be 95% confident that the values are more variable and fall between value x~1~...x~n~ which more variable, correct? If so, then it does not seem appropriate to use this simplified calculation which uses the standard error and another method is needed. 
