...
```{r robustsamplesize,  include=TRUE, message=FALSE, echo=TRUE}
# samples (n) needed for a rubust mean
sum_stat <- metadata2 %>% group_by(Soil) %>% rstatix::get_summary_stats(D1517, show = c("mean", "sd", "se"))
n = ( (2 * sum_stat$sd) / sum_stat$se )^2
sum_stat
...
```
At a fixed 95% confidence the Kurosol requires more samples as the SE is lower in this soil. So despite that the values in the Chromosol are more variable, less samples are apparently required. Thus one can be 95% confident that the values are more variable and fall between value x~0~...x~n~ which more variable, correct? If so, then it does seem appropriate to use this simplified calculation which uses the standard error and another method is needed. 
  
Here this same formula is applied to dieldrin losses (n= 12)
